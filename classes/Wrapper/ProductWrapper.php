...
class ProductWrapper
{
    ...
    public function prepareItemFromProduct($product, $useProvidedQuantity = false)
    {
        ...
        if (!empty($product['manufacturer_name'])) {
            ...
            $item['brand'] = $product['manufacturer_name'];
        ...
        if (!empty($product['id_product_attribute'])) {
            ...
            $item['product_id'] .= '-' . $product['id_product_attribute'];
            //$item['variant'] = $product['id_product_attribute'];
        }

        // Information about a chosen variant, if we have it (cart list has this out of the box)
        if (!empty($product['attributes_small'])) {
            $item['variant'] = $product['attributes_small'];
        ...
        foreach ($productCategories as $productCategory) {
            ...
            $item[$counter == 1 ? 'category' : 'category' . $counter] = $productCategory['name'];
            if ($counter == 5) {
                break;
            }
            ++$counter;
